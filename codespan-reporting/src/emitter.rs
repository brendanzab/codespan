...
impl<'a> MarkedSource<'a> {
    ...
    fn source(&self, span: Span) -> Option<&'a str> {
        self.files.source(self.label.file_span.id, span)
    }
    ...
    fn emit(&self, writer: &mut impl WriteColor, config: &Config) -> io::Result<()> {
        ...
        let source_prefix = self.source(prefix_span).expect("prefix");
        ...
        let mark_len = if start.line == end.line {
            ...
            let marked_source = self.source(self.span()).expect("marked_source");
            ...
            let marked_source = self.source(marked_span).expect("start_of_marked");
            writer.set_color(&label_spec)?;
            write!(writer, "{}", marked_source)?;
                ...
                .map(|i| LineIndex::from(i as u32))
            {
                ...
                let marked_source = self.source(mark_span).expect("marked_source");
                writer.set_color(&label_spec)?;
        ...
        let source_suffix = self.source(suffix_span).expect("suffix");
